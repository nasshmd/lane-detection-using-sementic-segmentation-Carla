...
def vis_lanes(image_out, lane_lines):
    for line in lane_lines:
        ...
        image_out = cv2.line(
            image_out.astype(
                np.uint8), (x1, y1), (x2, y2), (255, 0, 255), 2)
    return image_out
...
def merge_lane_lines(
    ...
    slope_similarity_threshold = 0.2
    ...
    merged_lines = lines[abs(slopes) > min_slope_threshold]
...
def make_carla_settings(args):
    ...
    camera0 = sensor.Camera('CameraSemSeg', PostProcessing='SemanticSegmentation')
...
class CarlaGame(object):
    ...
    def _on_render(self):
        ...
        if self._main_image is not None:
            ...
            result[np.where(array == 6)] = [255, 16, 59]
            result[np.where(array == 8)] = [255, 10, 56]

            kernel = np.ones((5, 5), np.uint8)
            erosion = cv2.dilate(np.uint8(result), kernel, iterations=1)
            dilate = cv2.erode(erosion, kernel, iterations=1)
            edges = cv2.Canny(dilate, 50, 140)

            # Step 3: Perform Line estimation using cv2.HoughLinesP()
            params = dict(rho=10, theta=np.pi / 180, threshold=100, minLineLength=200, maxLineGap=50)
            ...
            if lines is not None:
                ...
                max_y = edges.shape[0]/2
                min_y = max_y/2

                merged_lines = merge_lane_lines(lines)
                #extrapolated_lanes = extrapolate_lines(merged_lines, max_y, min_y)
                result = vis_lanes(result, merged_lines)
